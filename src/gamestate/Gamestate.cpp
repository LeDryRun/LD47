...
int Gamestate::get_gui_button_index(std::string name_p){
	for(int i=0;i<(int)gui_layer_buttons.size();i++){
		if(gui_layer_buttons.at(i)->get_name()==name_p){
			return i;
		}
	}
	std::cout<<"ERROR: unable to find button "<<name_p<<std::endl;
	return -1;
}
