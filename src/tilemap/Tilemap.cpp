...
void Tilemap::load_from_file(std::string file_p){
	//std::cout<<"tilemap::load_from_file"<<std::endl;
	...
	//	std::cout<<"map:"<<map.at(i).get_map_x()<<","<<map.at(i).get_map_y()<<":"<<map.at(i).get_tile_type()<<"="<<map.at(i).get_type_s()<<std::endl;
	//}
	//std::cout<<std::endl;
...
void Tilemap::load_tile(int x_p, int y_p, std::string load_p){
	//std::cout<<"tilemap::load_tile"<<std::endl;
	map.at(x_p+y_p*length_x).load(x_p,y_p,tileset,load_p);

	sf::Vertex* quad = &vertex_array[(x_p+(y_p*length_x))*4];

	set_tile_rotation(x_p,y_p,map.at(x_p+y_p*length_x).get_rotation_int());

	if(map.at(x_p+y_p*length_x).is_visible()){
		int tile_type=map.at(x_p+y_p*length_x).get_type_image_int();
		int tu= (tile_type % tileset.get_size_x());
		int tv= (tile_type / tileset.get_size_x());

		quad[0].texCoords=sf::Vector2f(tu*(tileset.get_tile_width())+((tu+1)*tileset.get_buffer_x()),tv*tileset.get_tile_height()+((tv+1)*tileset.get_buffer_y()));
		quad[1].texCoords=sf::Vector2f((tu+1)*tileset.get_tile_width()+((tu+1)*tileset.get_buffer_x()),tv*tileset.get_tile_height()+((tv+1)*tileset.get_buffer_y()));
		quad[2].texCoords=sf::Vector2f((tu+1)*tileset.get_tile_width()+((tu+1)*tileset.get_buffer_x()),(tv+1)*tileset.get_tile_height()+((tv+1)*tileset.get_buffer_y()));
		quad[3].texCoords=sf::Vector2f(tu*tileset.get_tile_width()+((tu+1)*tileset.get_buffer_x()),(tv+1)*tileset.get_tile_height()+((tv+1)*tileset.get_buffer_y()));
		//quad[0].color=sf::Color(0,0,0);
		//quad[1].color=sf::Color(0,0,0);
		//quad[2].color=sf::Color(0,0,0);
		//quad[3].color=sf::Color(0,0,0);
	}else{
...
void Tilemap::set_tile_rotation(int x_p, int y_p, int rot_p){
	//std::cout<<"tilemap::set_tile_rotation"<<std::endl;
	sf::Vertex* quad = &vertex_array[(x_p+(y_p*length_x))*4];

	map.at(x_p+y_p*length_x).set_rotation_int(rot_p);

	quad[(0+4-rot_p)%4].position=sf::Vector2f(x_p* tileset.get_tile_width(),y_p*tileset.get_tile_height());
	quad[(1+4-rot_p)%4].position=sf::Vector2f((x_p+1)* tileset.get_tile_width(),y_p*tileset.get_tile_height());
	quad[(2+4-rot_p)%4].position=sf::Vector2f((x_p+1)* tileset.get_tile_width(),(y_p+1)*tileset.get_tile_height());
	quad[(3+4-rot_p)%4].position=sf::Vector2f(x_p* tileset.get_tile_width(),(y_p+1)*tileset.get_tile_height());
	//std::cout<<"~tilemap::set_tile_rotation"<<std::endl;
}

void Tilemap::set_tile_type(Point p_p, std::string s_p){
	load_tile(p_p.get_x(),p_p.get_y(),std::to_string(tileset.type_s_to_int(s_p)));
}
